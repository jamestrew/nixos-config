# Vendored from nixpkgs PR #538312 (services.nordvpn), which is not yet merged.
# https://github.com/NixOS/nixpkgs/pull/538312
# Uses the official `pkgs.nordvpn` package (nixpkgs PR #477174).
# Remove this file and use the upstream `services.nordvpn` module once it lands
# in a nixos-unstable channel bump. The upstream `meta = { doc = ./nordvpn.md; }`
# block is dropped here because we do not vendor the manual doc.
{
  config,
  lib,
  pkgs,
